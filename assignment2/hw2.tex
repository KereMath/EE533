...
Since $p_Y(0)=0$ but $p_X(0)=\tau>0$, the $x=0$ term diverges:
\[
  p_X(0)\ln\frac{p_X(0)}{p_Y(0)} = \tau\ln\frac{\tau}{0} = +\infty.
\]
...
We show $H(X_0\mid X_n)\le H(X_0\mid X_{n+1})$ for every $n\ge 0$.

The Markov property gives $X_0\perp X_{n+1}\mid X_n$, so conditioning on $X_{n+1}$ adds nothing about $X_0$ once $X_n$ is known:
\[
  H(X_0\mid X_n,\;X_{n+1}) = H(X_0\mid X_n). \tag{1}
\]
Conditioning on fewer variables cannot decrease entropy:
\[
